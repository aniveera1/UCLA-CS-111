...
void generate_temp_report(int temp_pin, char* scale, char* log_file)
{
	...
	SSL_write(ssl_struct, temp_report, 14);
...
void process_command(char* command, char** scale, int* delay, int* report, const char* log_file)
{
	...
	else if (strstr(command, "OFF") != NULL)
	{
		...
		SSL_write(ssl_struct, curr_time, 19);
...
int connect_to_server(const char* host, const char* port)
{
	...
	SSL_CTX* ssl_framework = SSL_CTX_new(ssl_connection);
	if (ssl_framework == NULL)
	{
		fprintf(stderr, "%s\n", "SSL_CTX_new failed.");
		exit(FAIL_CODE);
	}

	ssl_struct = SSL_new(ssl_framework);
	if (ssl_struct == NULL)
	{
		fprintf(stderr, "%s\n", "SSL_new failed.");
		exit(FAIL_CODE);
	...
	if (SSL_set_fd(ssl_struct, sockfd) == 0)
	{
		fprintf(stderr, "%s\n", "SSL_set_fd failed.");
		exit(FAIL_CODE);
	}

	if (SSL_connect(ssl_struct) < 1)
	{
		fprintf(stderr, "%s\n", "SSL_connect failed.");
		exit(FAIL_CODE);
	}
...
int main(int argc, char** argv)
{
	...
	temp_buff[2] = '=';
	int temp_id = atoi(id);
	strcpy(temp_buff+3, id);
	temp_buff[12] = '\n';
	temp_buff[13] = '\0';

	SSL_write(ssl_struct, temp_buff, 13);
	...
	while (1)
	{
		...
		{
			int bytes_read = SSL_read(ssl_struct, buf, sizeof(buf));
			if (bytes_read < 0)
			{
